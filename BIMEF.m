function fused = BIMEF(I, mu, k, a, b)
...
%     figure("name", "scene illumination map");
%     subplot(1,1,1);imshow(t_our);title("t_0");
...
%     figure("name", "exposure ratio");
%     subplot(1,1,1);imshow(J);title("J");
    ...
    figure("name", "weighted matrix");
    subplot(3,1,1);imshow(I2);title("I2");
    subplot(3,1,2);imshow(J2);title("J2");
    subplot(3,1,3);imshow(fused);title("fused");
    
        function J = maxEntropyEnhance(I, isBad)
            Y = rgb2gm(real(max(imresize(I, [50 50]), 0))); % max - avoid complex number 
            figure("name", "max entropy enhance");
            subplot(3,1,1);imshow(Y);title("Y");
